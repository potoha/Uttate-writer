...
このブランチは **Project B / API Provider Branch** です。ローカル小型モデル用のStage分割パイプラインはいったんMVP本線から外し、Gemini APIやOpenAI APIを差し替え可能なProviderとして扱う設計へ整理しています。現在は main 由来のStage 1読み転写だけを `local_ai` Provider として戻しています。

現在の実装状態:

- 標準Providerは `local_ai`
- `Enter` でチャンクを送信し、UIを止めずに変換
- `run.bat` でローカル環境から起動
- Gemini Providerは実装済み
- OpenAI Providerは実装済み
- main由来のローカルAI読み転写Provider `local_ai` を追加
- UI上部で Local AI / OpenAI / Google Gemini を切り替え可能
...
- Local AIはLM StudioのOpenAI互換APIを接続先として使います。既定では `http://127.0.0.1:1234/v1` を見ます。
