...
このリポジトリは、Uttate Writerの現在の開発本体です。2026年7月6日に旧
Project B / API Provider Branchをmainへ統合しており、以後はこのリポジトリの
現行仕様と現行計画に従って開発します。

旧Project Bの計画書・ブランチ移行資料は、判断の経緯を残すために保持しています。
これらは現行の実装指示ではありません。現在の変更は[プロダクト仕様](project.md)と
[安全性・保守性リファクタリング計画](docs/REFACTORING_PLAN.md)を優先してください。
...
- [プロダクト仕様（現行）](project.md)
- [安全性・保守性リファクタリング計画（現行の実装順序）](docs/REFACTORING_PLAN.md)
- [API前処理・送信フロー（現行）](docs/API_PREPROCESSING_FLOW.md)
- [Dataset Builder Add-on](docs/DATASET_BUILDER.md)

### 履歴資料

- [Project B計画（履歴）](project-b-api.md)
- [Project B MVPスコープ（履歴）](docs/PROJECT_SCOPE.md)
- [Project B実装計画（履歴）](docs/IMPLEMENTATION_PLAN.md)
- [main統合準備メモ（履歴）](docs/MAIN_MERGE_PREP.md)
- [変換モジュール化の移行記録（履歴）](docs/CONVERSION_MODULARIZATION.md)
