...
- テスト専用Providerによるネットワークなしの開発・検証
...
4. Local AI / Gemini / OpenAIを切り替えられるProvider UI
...
12. テストダブル中心の自動テストと、Gemini/OpenAI/Local AI実接続用の手動またはopt-inテスト
...
- `local_ai`: main由来のStage 1読み転写をLocal AIとして実行する標準Provider。接続先にはLM StudioのOpenAI互換APIを使う。
...
```text
UTTATE_PROVIDER=local_ai|gemini|openai
GEMINI_API_KEY=
...
- テストダブルまたはContract Testで契約を検証できる。
...
2. Local AIまたはテストダブルで各チャンクが変換され、候補/原文をレビューできる。
3. GeminiProviderで少なくとも代表入力3件を実API変換できる。
4. OpenAIProviderで同じProvider contractを満たす。
5. Providerをlocal_ai/gemini/openaiで切り替えてもUIコードとChunkモデルが分岐しない。
...
10. READMEにLocal AI/Gemini/OpenAIの起動方法、API利用時の送信データ注意、テスト方法が記載されている。
